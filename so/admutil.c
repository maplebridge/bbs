...
a_system_setup()
{
  ...
  vget(++i, 0, "站簽全站指定(按 0 取消指定)：", buf, 10, GCARRY);
  host_sight_select_tmp = atoi(buf);
  if(host_sight_select_tmp != 0)
  {
    if (host_sight_select_tmp < 0)
    {
      vmsg("指定站簽數不得小於等於 0, 更改指定為 1 ");
  ...
  vget(++i, 0, "站簽娃使用全站指定(按 0 取消指定)：", buf, 10, GCARRY);
  model_select_tmp=atoi(buf);
  if (model_select_tmp !=0)
  {
    if (model_select_tmp < 0)
    {
      vmsg("站簽娃指定編號不得小於等於 0, 更改指定為 1 ");
      model_select_tmp = 1;
    }
