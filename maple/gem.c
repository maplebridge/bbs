  ...
  int mode;
{
  ...
  /* itoc.000319: 限制級文章保密 */
  if ((xmode & GEM_RESTRICT) && !(level & GEM_M_BIT))
    prints("%-*.*s%s", d_cols + 67, d_cols + 66, "<資料保密>", mode ? "\033[m" : "");
  else if (gtype == 0)
  ...
  XO *xo;
{
  ...
  /* smiler.080614 : 僅板主及小板主可複製精華區 */
  int level;
  level = xo->key;
  if (!(level & GEM_W_BIT))
    return XO_NONE;

  gem_buffer(xo->dir, tag ? NULL : (HDR *) xo_pool + (xo->pos - xo->top), gem_chkrestrict ,1);
  ...
  int pos;		/* -1: 附加在最後  >=0: 貼上的位置 */
{
  ...
  if (xmode & GEM_FOLDER)	/* 卷宗/分類 */
  {
    ...
    if (data = (HDR *) f_img(folder, &fsize))
    {
      ...
      do
      {
	/* 只有 gem_copy() 才可能有 GEM_FOLDER，所以用精華區的 gem_chkrestrict */
	if (gem_chkrestrict(head))
