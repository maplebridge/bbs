...
void brh_get(time4_t bstamp, int bhno);
int brh_unread(time4_t chrono);
void brh_visit(int mode);
int brh_add(time4_t prev, time4_t chrono, time4_t next);
int bstamp2bno(time4_t stamp);
void brh_save(void);
void brd_force(void);
...
int post_t_score(XO *xo,char *log, HDR *hdr_in);
...
int u_menu_bar(void);
int menu_bpg_color_bar(void);
int menu_pl_color_bar(void);
int menu_wma_color_bar(void);
int menu_vns_color_bar(void);
int u_rss_bar(void);
...
void outsxy(uschar *sstr, int x, int y);
...
void line_save(int num, char *my_line);
...
int Tagger(time4_t chrono, int recno, int op);
