...
#define BMW_FORMAT	COLOR1 "★ %s " COLOR3 " %s \033[m"	/* 收到的水球 */
#define BMW_FORMAT2	"To %s: %s "				/* 送出的水球 */
...
bmw_outz()
{
  ...
  if (bmw->nick[0] != '\0')
    prints(MSN_FORMAT2,
      bmw->nick, i >= 0 ? benz->msg : "【您最近沒有傳 MSN 訊息給這位使用者】");
  else
#endif
...
bmw_log()
{
  ...
  if (!stat(fpath, &st) && S_ISREG(st.st_mode))
  {
    ...
    if (!st.st_size)		/* itoc.000512: 不儲存水球記錄 */
