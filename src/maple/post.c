...
post_filter(fpath, title)	/* smiler.080830 : 看門狗對文章內容過濾 */
...
#ifdef DO_POST_FILTER
  if (post_filter(fpath, ve_title))	/* smiler.080830: 針對文章標題內容偵測有無不當之處 */
  {
    unlink(fpath);
    pcurrhdr = NULL;
    return XO_HEAD;
  ...
  XO *xo;
{
  ...
  method = (HAS_PERM(PERM_ALLBOARD) || (!tag && !strcmp(hdr->owner, cuser.userid))) &&
    (HAS_PERM(PERM_ALLBOARD) || comefrom != 0) && (vget(2, 0, "(1)原文轉載 (2)轉錄文章？[2] ", buf, 3, DOECHO) == '1') ? 0 : 1;
  ...
  do	/* lkchu.981201: 整批轉錄 */
  {
	...
	Now(), cuser.userid, comefrom ? (method ? tmpboard : currboard) : "個人信箱", ve_title);
      f_cat(fpath_log, content_log);
    ...
    if (IS_BRD_DOG_FOOD(fpath, xboard))
    {
      ...
      sprintf(fpath_log, FN_ETC_BBSDOG_LOG);
      sprintf(content_log, "%s 文章內容限制: 轉錄失敗\n作者: %s\n來源: %s\n看板: %s\n標題: %s\n字串: %s\n\n", Now(), cuser.userid, comefrom ? (method ? tmpboard : currboard) : "個人信箱", xboard, ve_title, bbs_dog_str);
      f_cat(fpath_log, content_log);
  ...
  char *fpath;
{
  ...
  else if (hdr->parent_chrono)	/* 此篇文章有置底副本，一併更改之 */
  {
    ...
    {
      unlink(xpath);
      f_ln(fpath, xpath);	/* 重設 link, 以使內文同步 */
    }
  }
}
#endif
  ...
  char *fpath;
{
  ...
  f_cp(fpath, tmp, O_TRUNC);	/* tmp 存備份，並且拿來做編輯 */

  /* 拿 tmp 下去做編輯，而非原始檔 */
#ifdef DO_POST_FILTER	/* smiler.080830: 針對文章標題內容偵測有無不當之處 */
  ...
  system(cmd);
  backup_post_log(patch, BN_EDITLOG, hdr, 0);	/* 記錄修改前後差異 */
#else
//  backup_post_log(fpath, BN_EDITLOG, hdr, 0);	/* 修改前的原始文章備份 */
//  backup_post_log(tmp, BN_EDITLOG, hdr, 0);	/* 把修改過後的文章備份 */
#endif

  if (!stat(fpath, &st) && (stamp == st.st_size))	/* 原始檔案未被修改: 沒有新推文 */
  {
    unlink(fpath);
    f_mv(tmp, fpath);
  }
  ...
  unlink(ori);
  unlink(patch);
  post_bottom_sync(hdr, fpath);	/* 修改文章後，試著將原文與置底文同步 */
...
#ifdef DO_POST_FILTER
  if (post_filter(tmpfile, tmptitle))	/* smiler.080830: 針對文章標題內容偵測有無不當之處 */
  {
    unlink(tmpfile);
    return XO_HEAD;
  }
