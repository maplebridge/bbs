...
/* 保留帳號期限 -- 學期中 */
//#define DAY_NEWUSR	180	/* 登入不超過三次的使用者保留 180 天 */
#define DAY_FORFUN	30	/* 未完成身分認證的使用者保留  30 天 */
#define DAY_OCCUPY	180	/* 已完成身分認證的使用者保留 180 天 */

/* 保留帳號期限 -- 暑假 */
//#define VAC_NEWUSR	180	/* 登入不超過三次的使用者保留 180 天 */
//#define VAC_FORFUN	30	/* 未完成身分認證的使用者保留  30 天 */
...
//  if (ulevel & (PERM_XEMPT | PERM_BM | PERM_ALLADMIN))	/* 有這些權限者不砍 */
  if (ulevel & (PERM_XEMPT | PERM_ALLADMIN))        /* songsongboy.070404: 因個人版因素故砍版主 */
  {
    datemsg(buf, &acct.lastlogin);
    levelmsg(data, ulevel);
    fprintf(flst, "%5d) %-13s%s[%s]\n", userno, acct.userid, buf, data);
    manager++;

#ifdef CHECK_LAZYBM
    if ((ulevel & PERM_BM) && life < due_lazybm)
    {
      fprintf(fbm, "%5d) %-13s%s\n", userno, acct.userid, buf);
      lazybm++;
    }
#endif
  }
